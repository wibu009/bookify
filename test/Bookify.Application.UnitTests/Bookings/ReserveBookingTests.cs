...
public class ReserveBookingTests
{
    ...
    private static readonly ReserveBookingCommand Command = new(
        ...
        new DateOnly(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day).AddDays(10));
    ...
    [Fact]
    public async Task Handle_Should_ReturnFailure_WhenApartmentIsNotFound()
    {
        // Arrange
        _userRepositoryMock
            .GetByIdAsync(Command.UserId, Arg.Any<CancellationToken>())
            .Returns(User.Create(UserData.FirstName, UserData.LastName, UserData.Email));
        _apartmentRepositoryMock
    ...
    public async Task Handle_Should_CallRepository_WhenBookingIsReserved()
    {
        ...
        _bookingRepositoryMock
            .Received(1)
            .Add(Arg.Is<Booking>(booking =>
                booking.Id == result.Value));
    }
}